...
"#;
    ...
    assert_eq!(
        parse_structural_job(&canonical).unwrap().canonical_document(),
        canonical
    );
}
...
fn unsupported_analysis_and_property_or_temporal_directives_fail_closed() {
    let invalid =
        parse_structural_job("analysis \"safety\"\nmodel \"graph.fvl\"").unwrap_err();
    assert!(matches!(
        invalid.kind(),
...
fn duplicate_singletons_empty_path_bad_numbers_and_trailing_input_are_rejected() {
    let duplicate = parse_structural_job(
        "analysis \"recurrence\"\nmodel \"a.fvl\"\nmax-depth 1\nmax-depth 2",
    )
    .unwrap_err();
    assert!(matches!(
    ...
    let number = parse_structural_job(
        "analysis \"recurrence\"\nmodel \"a.fvl\"\nmax-states nope",
    )
    .unwrap_err();
    assert!(matches!(
    ...
    let trailing = parse_structural_job(
        "analysis \"recurrence\"\nmodel \"a.fvl\" trailing",
    )
    .unwrap_err();
    assert!(matches!(
        trailing.kind(),
