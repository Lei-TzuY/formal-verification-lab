...
fn fixture_dir(kind: &str) -> PathBuf {
    ...
    let root = std::env::temp_dir().join(format!(
        "fvlab-m63-six-family-suite-{kind}-{}-{id}",
        std::process::id()
    ));
...
fn write_deadlock_job(root: &Path) -> PathBuf {
    fs::write(
        root.join("deadlock.fvl"),
        "model \"deadlock-bad\"\nstate \"start\"\nstate \"bad\"\nstate \"declared-legitimate\"\ninitial \"start\"\nedge \"start\" \"stop\" \"bad\"\nlabel \"declared-legitimate\" \"legitimate\"\n",
    )
    .unwrap();
    fs::write(root.join("deadlock.fvp"), "\"legitimate\"\n").unwrap();
    let manifest = root.join("deadlock.fvj");
    fs::write(
        &manifest,
        "analysis \"deadlock\"\nmodel \"deadlock.fvl\"\nproperty \"deadlock.fvp\"\n",
    )
    .unwrap();
    manifest
}
...
fn create_jobs(root: &Path) -> [PathBuf; 6] {
    [
        write_legacy_multi_response_job(root),
        write_safety_job(root),
        write_deadlock_job(root),
...
#[test]
fn raw_six_family_suite_preserves_direct_envelopes_order_and_aggregate_precedence() {
    let root = fixture_dir("raw");
    let jobs = create_jobs(&root);
    let suite = write_suite(&root, "six-family-raw", false, "violated");

    let run = run_verification_suite_json(&suite);
    assert_eq!(run.exit_code, 7);
    assert_eq!(run.envelope.outcome, VerificationSuiteOutcome::Violated);
    assert_eq!(run.envelope.jobs.len(), 6);
    ...
    assert_eq!(
        run.envelope.jobs[5].result.analysis.as_deref(),
        Some("action-temporal")
    );
    assert_eq!(
        run.to_json(),
        run_verification_suite_json(&suite).to_json(),
        "six-family raw suite output must remain deterministic"
    );
...
#[test]
fn six_family_expectations_match_and_deadlock_mismatch_is_not_reinterpreted() {
    let root = fixture_dir("expectations");
    create_jobs(&root);

    let matched_suite = write_suite(&root, "six-family-matched", true, "violated");
    ...
    assert_eq!(matched.envelope.jobs.len(), 6);
    ...
    let mismatch_suite = write_suite(&root, "six-family-mismatch", true, "satisfied");
    ...
    assert!(mismatch.envelope.jobs[..2]
        .iter()
...
#[test]
fn built_binaries_preserve_all_six_direct_envelopes_in_raw_and_expectation_suites() {
    let root = fixture_dir("cli");
    let jobs = create_jobs(&root);
    let suite = write_suite(&root, "six-family-cli", true, "violated");
    let names = [
        "legacy.fvj",
        "safety.fvj",
        "deadlock.fvj",
