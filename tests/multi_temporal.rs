...
fn parser_fails_closed_on_structure_and_semantics() {
    let unknown =
        parse_multi_response_temporal("bad", r#"eventually("class-a","request-a","grant-a")"#)
            .unwrap_err();
    assert!(matches!(
        unknown.kind(),
    ...
    let arity =
        parse_multi_response_temporal("bad", r#"response("class-a","request-a")"#).unwrap_err();
    assert!(matches!(
        arity.kind(),
    ...
    let empty_action =
        parse_multi_response_temporal("bad", r#"response("class-a","   ","grant-a")"#).unwrap_err();
    assert!(matches!(
        empty_action.kind(),
...
"#,
    ...
    assert!(matches!(
        empty.kind(),
        MultiResponseTemporalParseErrorKind::Semantic(MultiResponseTemporalSpecError::NoClauses)
    ));
}
...
"#,
    ...
    assert!(cycle
        .iter()
        .any(|step| step.action.as_deref() == Some("wait-b")));
