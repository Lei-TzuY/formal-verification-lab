use formal_verification_lab::{
    check_buchi_with_fairness_profile, check_buchi_with_fairness_profile_and_product_limits,
    check_monitor_with_fairness_profile, check_monitor_with_fairness_profile_and_product_limits,
    AcceptanceSet, BoundedOutcome, BuchiAutomaton, BuchiCounterexample, BuchiProductState,
    BuchiStatus, ExplorationLimits, FairnessProfile, FiniteMonitor, FiniteRunPolicy, Invariant,
    MonitorCounterexample, MonitorProductState, MonitorStatus, ProgressCondition, RejectCondition,
    StateVariable, TraceStep, Transition, TransitionSystem,
};
...
fn generated_model(graph_mask: usize, codes: [u8; EDGE_COUNT]) -> TransitionSystem<usize> {
    TransitionSystem::new(
        format!("combined-fair-monitor-differential-{graph_mask}"),
...
fn generated_monitor() -> FiniteMonitor<bool> {
    FiniteMonitor::new(
        "generated-combined-fair-monitor",
        true,
...
fn generated_buchi() -> BuchiAutomaton<bool> {
    BuchiAutomaton::new(
        "generated-combined-fair-monitor-buchi",
        true,
...
fn all_two_node_products_match_direct_combined_fair_buchi() {
    ...
    for graph_mask in 0..(1usize << EDGE_COUNT) {
        for assignment in 0..ASSIGNMENT_COUNT {
            ...
            let monitor_result =
                check_monitor_with_fairness_profile(&model, &monitor, &profile).unwrap();
            let buchi_result =
                check_buchi_with_fairness_profile(&model, &automaton, &profile).unwrap();
    ...
    for graph_mask in 0..(1usize << EDGE_COUNT) {
        for assignment in 0..ASSIGNMENT_COUNT {
            ...
            for limit in BOUNDED_LIMITS {
                ...
                let monitor_result = check_monitor_with_fairness_profile_and_product_limits(
                    &model, &monitor, &profile, limits,
                )
                .unwrap();
                let buchi_result = check_buchi_with_fairness_profile_and_product_limits(
