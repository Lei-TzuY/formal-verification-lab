...
#[test]
fn weak_fair_close_filters_only_the_unfair_monitor_progress_lasso() {
    ...
    assert!(fair_text.contains("weak fairness actions: 1"));
    assert!(fair_text.contains("weak-fair action: \"close\""));
    ...
    assert!(unrelated_text.contains("weak-fair action: \"unrelated\""));
}

#[test]
fn weak_fairness_never_excuses_rejecting_or_finite_terminal_monitor_violations() {
    let rejecting = run(&[
        "monitor",
        "session-double-open",
        "--weak-fair-action",
...
#[test]
fn bounded_weak_fair_monitor_cli_preserves_cutoff_honesty_and_exit_three() {
    let product = run(&[
        "monitor",
        "session-unfair-close",
...
#[test]
fn monitor_fairness_options_fail_closed_on_duplicate_empty_or_missing_actions() {
    let duplicate = run(&[
        "monitor",
        "session-unfair-close",
        "--weak-fair-action",
