...
The next production frontier is **Milestone 90: versioned portable workspace snapshot and replay**. M89 can now execute a complete logical workspace from supplied text sources, but those sources are still external to the execution request. M90 should deterministically capture one orchestration root and its explicit transitive family-manifest/model/property/certificate closure into a versioned length-framed UTF-8 snapshot, replay it through M89's map provider without host I/O, and prove byte-identical creation/replay across different host roots and in-memory providers. It must remain a reproducibility artifact, not a cryptographic integrity or authenticity claim.
