...
The repository now has one coherent explicit-state stack from finite transition-system construction through safety, reachability, recurrence, liveness, single- and multi-response obligations, finite-monitor progress/rejection semantics, generalized Büchi acceptance, textual/declarative frontends, deterministic model/product resource budgets, deep-graph SCC traversal, opt-in exact-action weak fairness, opt-in exact-action strong fairness, and an explicit combined weak/strong fairness profile at the generalized Büchi core. Strong fairness is available for generalized Büchi, single-response/action-temporal, multi-response, and finite-monitor verification across unbounded, product-bounded, and staged APIs. M45 adds a typed `FairnessProfile` whose mixed execution filter requires every configured weak and strong obligation simultaneously while preserving exact no-fair, weak-only, and strong-only compatibility paths.

Historical no-fairness behavior remains the default. Fairness is enabled only when explicitly supplied, and it constrains infinite executions only. Product and staged cutoffs remain proof-honest: missing prefix edges are never treated as proof that an action is disabled, and unresolved enablement provenance produces `INCONCLUSIVE` rather than a false proof or counterexample. M45 seals mixed weak-plus-strong semantics at the typed generalized Büchi core; the existing CLI still fails closed on mixed assumptions until that sealed profile is deliberately propagated through bounded/staged and frontend layers.
...
- M45 provides explicit combined weak/strong fairness semantics through the typed unbounded generalized Büchi API. Mixed CLI input still fails closed because bounded/staged composition and frontend routing have not yet been promoted to that profile; no dispatch precedence silently substitutes for combined semantics.
...
Milestones 1–45 now form a coherent explicit-state stack: safety and bounded honesty -> typed models and independent graph validation -> reachability/deadlock/recurrence -> eventuality and response obligations -> finite monitors and generalized Büchi acceptance -> shared graph/product substrates -> typed/textual/declarative specification frontends -> bounded state properties -> product/staged temporal budgets -> iterative deep-graph SCC traversal -> opt-in exact-action weak fairness -> bounded/staged enablement provenance -> weak-fair response and finite-monitor composition -> direct weak-fair monitor integration -> opt-in exact-action strong fairness -> proof-honest bounded/staged strong-fair Büchi verification -> strong-fair response/temporal/multi-response/monitor composition -> direct strong-fair CLI integration -> explicit combined weak/strong fairness at the generalized Büchi recurrent core.

M45 seals the unbounded mixed-fairness semantics. The next highest-value architectural slice is **Milestone 46: proof-honest product-bounded and staged combined fairness**. The goal is to carry the same `FairnessProfile` through deterministic product limits and independent model/product stages before exposing mixed assumptions through higher-level response, monitor, temporal, or CLI surfaces.

Acceptance criteria for M46:

- preserve exact no-fair, weak-only, strong-only, and unbounded mixed-profile results under generous limits;
- reuse the existing bounded/staged product construction and action-enablement provenance instead of introducing another traversal or cutoff engine;
- treat unknown enablement conservatively for both fairness classes, so missing prefix edges never prove a weak action disabled or a strong action absent;
- preserve finite-terminal precedence and return a mixed infinite violation only when the retained prefix contains a real closed walk satisfying every weak and strong obligation simultaneously;
- return stage-qualified `INCONCLUSIVE` with the exact model/product state/transition/depth reason whenever the proof or fairness evidence is incomplete;
- add independent small-graph × limit differential validation covering mixed assumptions, overlap canonicalization, cutoff-before/after-witness behavior, and deterministic accounting;
- keep mixed CLI/frontend routing fail-closed until the bounded/staged core is sealed;
