...
M68 adds a validated partial-order reduction path for safety checking. Raw action-independence declarations remain non-authoritative, while `validate_independence` binds accepted exact action-pair claims to one complete canonical reachable snapshot and its invariant observations. The standalone reduced checker accepts only that validated evidence and preserves distinct sleep contexts for revisited model states. This is a proof-safety foundation rather than a scalability claim: validation itself exhaustively captures the reachable graph, and no result in this milestone treats CI timing as a performance benchmark.
...
| M68 | Validated safety partial-order reduction with fail-closed independence evidence and sleep-context-preserving exploration. |
| M69 | Typed CTL branching-time fixpoint kernel with explicit terminal totalization, memoized state-set semantics, deterministic evidence, and generated path oracles. |
| M70 | Declarative textual CTL parser, named-proposition binding, deterministic reporting, and direct file CLI over the sealed M69 authority. |
...
src/ctl.rs                    typed CTL formulas, fixpoint state sets, terminal policy, evidence
src/ctl_bounded.rs            lower/upper bounded CTL semantics + cutoff-safe evidence
src/ctl_parse.rs              textual CTL grammar + canonical rendering
src/declarative_ctl.rs        named-proposition binding onto the typed CTL authority
src/declarative_ctl_report.rs deterministic complete/bounded CTL reporting
...
src/mu_parse.rs                textual μ-calculus grammar + canonical rendering
src/declarative_mu.rs          named-proposition binding onto the typed μ authority
src/declarative_mu_report.rs   deterministic complete-graph μ-calculus reporting
...
src/verification_ctl.rs       CTL verification-job dispatch over sealed complete/bounded frontends
...
- `14`: declarative CTL property is conclusively `VIOLATED`;
- `15`: declarative modal μ-calculus property is `VIOLATED`;
...
- M69: **90,112** generated CTL formula evaluations: 4,096 graph/unary-valuation cases × 6 unary operators plus 32,768 graph/binary-valuation cases × 2 until operators.
- M71: **12,288** generated bounded-CTL soundness cases over all two-state graphs, proposition valuations, eight CTL operators, and six deterministic limit profiles.
- M77: **15,360** independent bounded modal μ-calculus soundness cases over all 16 directed two-state graphs, 16 P/Q proposition valuations, 10 native μ-calculus formulas (including negation, lexical shadowing, and nested μ/ν alternation), and six deterministic limit profiles.
- M68: **32,768** deterministic three-state/two-action graph × safety-mask combinations drive validation; every relation accepted by the validator is differentially compared against canonical exhaustive safety, alongside focused invalid-declaration, invariant-observation, sleep-context revisit, and deterministic-witness regressions.
...
- M75 adds the typed complete-graph modal μ-calculus authority; M76 adds deterministic textual/declarative syntax, named-proposition binding, reporting, and direct file execution while delegating semantics exclusively to M75. M77 adds proof-honest bounded typed μ-calculus using lower/upper state sets and lower/upper lexical fixpoint environments over the canonical bounded graph capture. M78 exposes those bounds through the same declarative frontend and `mu file` CLI with explicit three-valued cutoff reporting and exits. M79 adds first-class reproducible μ verification jobs with schema-v4 complete/bounded details, and M80 confirms the existing generic suite layer already preserves those envelopes without a μ-specific orchestrator. M81 adds an independent complete-graph evaluation-game backend backed by a validated finite parity-game solver and a generated 3,584-case differential against M75. M82 exposes that backend through the same declarative parser/binder and `mu file --backend parity` while leaving no-option/default execution on the sealed fixpoint path. M83 carries explicit parity selection into verification-job manifests and schema-v4 envelopes while preserving fixpoint as the default and bounded authority. M84 adds deterministic Even/Odd positional strategy evidence to the generic parity solver plus an independent fail-closed verifier, with exhaustive certification over all 324 two-vertex total games in its generated gate. M85 maps those sealed strategies back into typed μ evaluation-game positions and semantic moves and independently reconstructs/verifies them against the original model+formula. M86 packages that evidence as a deterministic schema-v1 declarative parity certificate bound to canonical model/formula identity and independently verifies submitted artifacts without re-solving them; direct create/verify CLI commands expose the certificate surface. M87 adds a separate manifest-relative certificate-verification job protocol with deterministic `verified / rejected / error` JSON so artifact validity is not misreported as property truth; its runner verifies only the submitted artifact and never creates or re-solves a replacement certificate. M88 adds explicit family-tagged multi-family orchestration that preserves verification, structural, and certificate result envelopes without coercing their native outcomes, while separating operational `complete / attention / error` status from domain truth and regression expectations. M89 moves all three job families and M88 orchestration onto one logical UTF-8 text-source provider substrate, adds rooted-filesystem and map-backed execution, preserves path APIs as wrappers, rejects logical root escapes, and proves host-root-independent result JSON including error cases. M90 packages one explicit orchestration dependency closure into a deterministic length-framed schema-v1 UTF-8 snapshot and replays it entirely through the M89 map provider after host sources disappear, with cross-root/provider byte-identical creation and fail-closed closure/resource validation. M91 adds a versioned exact replay-result lock that freezes replay mode, exit code and full deterministic JSON and later compares current replay output byte-for-byte, so outcome-preserving accounting/evidence/schema drift is observable without reinterpreting family outcomes. M92 adds bounded deterministic structural diagnostics for those exact byte mismatches, identifying the first JSON-path drift when possible while preserving byte equality as the authority and falling back to byte-offset evidence for invalid/resource-limited JSON. Parity plus model-space bounds still fails closed rather than pretending bounded parity semantics. Only proven terminals are totalized; cut-state outgoing behavior remains unknown, and complete bounded captures collapse exactly to M75/M76 behavior. Structured replay-drift diagnostics, cryptographic authenticity/signatures, bounded parity semantics, μ fairness, symbolic fixpoint representations, and performance claims remain unsupported until their dedicated milestones are sealed.
- No milestone makes a performance claim from CI timing.

## Roadmap

Milestones 1–92 form the current explicit-state progression: canonical safety/bounded exploration -> typed finite models and independent graph validation -> reachability/deadlock/recurrence/eventuality -> response obligations, finite monitors and generalized Büchi acceptance -> shared graph/action-product substrates -> typed/textual/declarative property frontends -> proof-honest model/product budgets -> iterative deep-graph SCC traversal -> opt-in weak, strong and combined fairness -> reproducible verification/structural jobs and deterministic suites -> validated safety partial-order reduction evidence -> typed CTL branching-time fixpoints -> declarative/bounded CTL -> reproducible CTL jobs -> generic CTL suite integration -> typed modal μ-calculus -> textual/declarative μ-calculus -> proof-honest bounded μ-calculus -> bounded declarative μ execution -> reproducible μ verification jobs -> generic μ suite integration audit -> independent parity-game evaluation backend -> opt-in declarative parity execution -> reproducible parity-backed μ verification jobs -> certified positional parity strategies -> typed μ parity strategy evidence -> versioned declarative parity certificates -> reproducible certificate-verification jobs -> outcome-neutral multi-family orchestration -> host-independent virtual workspace execution -> versioned portable workspace snapshot/replay -> versioned exact replay-result lock -> structured exact-replay drift diagnostics.

M75 introduces the general typed fixpoint authority with lexical μ/ν binding and a 90,112-case CTL→μ cross-semantic differential. M76 exposes that authority through deterministic syntax, named propositions, stable reporting, and `fvlab mu file`. M77 makes arbitrary validated μ-calculus proof-honest under model-space cutoffs with lower/upper state sets and a 15,360-case independent bounded soundness oracle. M78 carries the same proof boundary through the declarative file CLI while preserving the no-option complete path and explicit 0/15/3/2 exit semantics. M79 carries both complete and bounded μ execution into the heterogeneous verification-job protocol with deterministic schema-v4 JSON. M80 confirms that the generic verification suite abstraction already preserves those μ envelopes. M81 adds a second complete-graph semantic path through finite parity games and a 3,584-case differential against M75. M82 exposes M81 through the same declarative frontend and an explicit `--backend parity` direct-file option while keeping fixpoint as the default and rejecting bounded parity requests. M83 carries that explicit backend selection into verification-job manifests and machine-readable schema-v4 results while retaining generic verification-suite compatibility and legacy fixpoint behavior. M84 extends the generic parity solver with deterministic positional strategies and independently verifies ownership, real-edge choices, region closure, totality, and recurrent parity. M85 maps those certified strategies back to normalized μ formula/model positions, semantic move kinds, and initial-root winners; its independent verifier rebuilds the canonical evaluation game and delegates recurrent strategy correctness back to M84. M86 turns that typed evidence into a deterministic versioned certificate bound to canonical declarative model/formula identity, verifies submitted artifacts without invoking the parity solver, and exposes direct certificate create/verify CLI commands while preserving M83 schema-v4 and ordinary `mu file` behavior. M87 adds a dedicated manifest-relative certificate-verification runner with deterministic schema-v1 `verified / rejected / error` results. M88 adds one family-tagged orchestration protocol that preserves each nested result envelope and separates operational execution status from family-native outcomes. M89 replaces host-filesystem coupling in those sealed execution paths with one logical text-source provider core, retaining filesystem compatibility while enabling fully in-memory mixed-family execution and byte-stable host-independent diagnostics. M90 freezes one explicit orchestration dependency closure into a bounded deterministic standalone snapshot, reverse-validates its embedded dependency set, and replays raw or expectation orchestration entirely from the embedded map-backed workspace. M91 freezes the corresponding process exit code and exact deterministic replay JSON in a versioned lock and detects current byte-level output drift without weakening coarse family-outcome semantics. M92 keeps that byte equality authoritative while adding bounded deterministic structural diagnostics for the first actionable JSON drift, including stable JSON Pointer paths and byte fallback.

The next production frontier is **Milestone 93: bounded deterministic replay audit**. Exact locks are most useful when one frozen snapshot replay is itself repeatable. M93 should execute the same M90 snapshot repeatedly under an explicit bounded attempt count, compare every run to the first with M91 exit/byte authority, reuse M92 diagnostics for the first mismatch, and expose deterministic/nondeterministic/error results without making timing, performance or probabilistic guarantees.
