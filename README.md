...
M46 seals combined fairness only at the generalized Büchi core. It does not yet expose mixed fairness through response, finite-monitor, action-temporal, or CLI routing, and it makes no performance claim.
...
- M24 state-property bounds and M25–M29/M33/M39–M46 staged temporal bounds are deterministic exploration budgets, not wall-clock deadlines or total-memory limits.
...
- M45/M46 provide explicit combined weak/strong fairness semantics through the typed generalized Büchi API across unbounded, product-bounded, and staged analysis. Mixed higher-level response/monitor/temporal and CLI routing still fails closed; no dispatch precedence silently substitutes for combined semantics.
...
Milestones 1–46 now form a coherent explicit-state stack: safety and bounded honesty -> typed models and independent graph validation -> reachability/deadlock/recurrence -> eventuality and response obligations -> finite monitors and generalized Büchi acceptance -> shared graph/product substrates -> typed/textual/declarative specification frontends -> bounded state properties -> product/staged temporal budgets -> iterative deep-graph SCC traversal -> opt-in exact-action weak fairness -> bounded/staged enablement provenance -> weak-fair response and finite-monitor composition -> direct weak-fair monitor integration -> opt-in exact-action strong fairness -> proof-honest bounded/staged strong-fair Büchi verification -> strong-fair response/temporal/multi-response/monitor composition -> direct strong-fair CLI integration -> explicit combined weak/strong fairness -> proof-honest product-bounded and staged combined-fair Büchi verification.

M46 seals mixed fairness at the generalized Büchi core across unbounded, product-bounded, and staged analysis. The next highest-value architectural slice is **Milestone 47: combined-fair single-response and action-temporal composition**. The goal is to propagate the already sealed `FairnessProfile` through the response adapter and typed/textual/declarative action-temporal frontend before exposing mixed assumptions through CLI dispatch.

Acceptance criteria for M47:

- compile single-response pending obligations and recurring-action temporal specifications into the existing M45/M46 combined-fair Büchi engines instead of adding a response- or frontend-specific recurrent traversal;
- expose unbounded, product-bounded, and staged combined-fair APIs while preserving historical response/temporal result types, backend identity, witness normalization, accounting, and exact stage-qualified `INCONCLUSIVE` reasons;
- preserve strict finite pending-terminal violations and the infinite-run-only finite policy of `all_infinitely_often` exactly;
- preserve exact no-fair, weak-only, and strong-only compatibility through the canonical `FairnessProfile` delegation paths;
- add differential composition regressions against direct M45/M46 Büchi construction, including mixed assumptions, overlap canonicalization, cutoff-before/after-witness behavior, generous-limit equality, and real declarative model-file integration;
- keep mixed CLI routing fail-closed until this typed/frontend composition is sealed;
- add no new temporal grammar, fairness-by-default behavior, traversal engine, wall-clock bound, or performance claim.
