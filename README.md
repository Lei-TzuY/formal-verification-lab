...
- M24 state-property bounds and M25–M29/M33/M39–M47 staged temporal bounds are deterministic exploration budgets, not wall-clock deadlines or total-memory limits.
...
- M45–M47 provide explicit combined weak/strong fairness semantics through generalized Büchi, single-response, and typed/textual/declarative action-temporal Rust APIs across unbounded, product-bounded, and staged analysis. Mixed temporal CLI dispatch, mixed multi-response, and mixed finite-monitor routing still fail closed; no dispatch precedence silently substitutes for combined semantics.
...
Milestones 1–47 now form a coherent explicit-state stack: safety and bounded honesty -> typed models and independent graph validation -> reachability/deadlock/recurrence -> eventuality and response obligations -> finite monitors and generalized Büchi acceptance -> shared graph/product substrates -> typed/textual/declarative specification frontends -> bounded state properties -> product/staged temporal budgets -> iterative deep-graph SCC traversal -> opt-in exact-action weak fairness -> bounded/staged enablement provenance -> weak-fair response and finite-monitor composition -> direct weak-fair monitor integration -> opt-in exact-action strong fairness -> proof-honest bounded/staged strong-fair Büchi verification -> strong-fair response/temporal/multi-response/monitor composition -> direct strong-fair CLI integration -> explicit combined weak/strong fairness -> proof-honest product-bounded and staged combined-fair Büchi verification -> combined-fair response and action-temporal composition.

M47 seals mixed fairness through the single-response adapter and typed/textual/declarative action-temporal frontend while intentionally leaving external mixed-assumption dispatch fail-closed. The next highest-value architectural slice is **Milestone 48: external combined-fair temporal CLI and reporting integration**.

Acceptance criteria for M48:

- allow simultaneous repeated `--weak-fair-action <ACTION>` and `--strong-fair-action <ACTION>` declarations on fixed temporal models, textual `temporal check`, and declarative `temporal file` routes;
- construct the canonical M45 `FairnessProfile`, preserving declaration order within each class and canonicalizing overlap to the strong class instead of rejecting or double-counting it;
- route unbounded, product-bounded, and staged temporal invocations through the sealed M47 APIs while preserving exact no-fair, weak-only, and strong-only compatibility behavior;
- render weak and strong assumptions explicitly and separately from canonical property evidence; preserve exit `0` for satisfaction, `10` for conclusive temporal violation, `3` for bounded/staged `INCONCLUSIVE`, and `2` for malformed fairness input;
- add built-binary regressions for mixed profile semantics, overlap canonicalization, fixed/textual/declarative routing, product/model cutoff provenance, and duplicate/empty/missing fairness arguments;
- keep mixed finite-monitor and multi-response composition outside M48, add no temporal grammar or fairness-by-default behavior, and introduce no second traversal engine, wall-clock proof bound, or performance claim.
