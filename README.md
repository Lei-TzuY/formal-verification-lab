...
- M24 state-property bounds and M25–M29/M33/M39–M48 staged temporal bounds are deterministic exploration budgets, not wall-clock deadlines or total-memory limits.
...
- M45–M48 provide explicit combined weak/strong fairness semantics through generalized Büchi, single-response, and typed/textual/declarative action-temporal APIs across unbounded, product-bounded, and staged analysis; M48 exposes mixed temporal assumptions on the CLI. Mixed multi-response and mixed finite-monitor composition still remain unavailable and are not silently approximated by dispatch precedence.
...
- M37 exposes finite-monitor weak fairness on the direct `monitor` CLI, and M44 exposes the sealed M43 strong-fair finite-monitor backend through the same direct command for unbounded, product-bounded, and staged analysis. Mixed monitor assumptions remain fail-closed.
...
Milestones 1–48 now form a coherent explicit-state stack: safety and bounded honesty -> typed models and independent graph validation -> reachability/deadlock/recurrence -> eventuality and response obligations -> finite monitors and generalized Büchi acceptance -> shared graph/product substrates -> typed/textual/declarative specification frontends -> bounded state properties -> product/staged temporal budgets -> iterative deep-graph SCC traversal -> opt-in exact-action weak fairness -> bounded/staged enablement provenance -> weak-fair response and finite-monitor composition -> direct weak-fair monitor integration -> opt-in exact-action strong fairness -> proof-honest bounded/staged strong-fair Büchi verification -> strong-fair response/temporal/multi-response/monitor composition -> direct strong-fair CLI integration -> explicit combined weak/strong fairness -> proof-honest product-bounded and staged combined-fair Büchi verification -> combined-fair response/action-temporal composition -> external combined-fair temporal CLI/reporting.

M48 seals external mixed weak/strong fairness for the temporal frontend while intentionally keeping multi-response and finite-monitor combined composition separate. The next highest-value architectural slice is **Milestone 49: combined-fair multi-response composition**.

Acceptance criteria for M49:

- add unbounded, product-bounded, and staged multi-response APIs that accept the canonical M45 `FairnessProfile` rather than inventing another mixed-fairness representation;
- preserve one generalized Büchi acceptance set per response clause so different pending classes can never alternate and mask starvation of one specific clause;
- preserve real finite pending-terminal violations because fairness constrains only infinite executions, and preserve exact violated-clause identity plus deterministic pending-vector witnesses;
- delegate empty, weak-only, and strong-only profiles exactly to the sealed no-fair, M35 weak-fair, and M42 strong-fair multi-response authorities; only genuinely mixed profiles may enter the M45/M46 combined-fair Büchi engines;
- preserve product/model cutoff accounting and model-before-product stage provenance; unknown enablement caused by a cutoff must remain `INCONCLUSIVE` rather than becoming evidence that a fair action is disabled;
- add differential compatibility regressions against existing no/weak/strong APIs plus focused genuinely mixed cases covering fairness filtering, real finite terminals, retained closed lassos, cutoff-before/after-witness behavior, and generous-limit equality;
- keep direct multi-response mixed-fairness CLI syntax outside this backend-composition slice unless it is deliberately promoted as a later vertical slice; add no fairness-by-default behavior, second traversal engine, wall-clock bound, or performance claim.
