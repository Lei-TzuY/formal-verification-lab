...
use formal_verification_lab::fairness_report::{
    ...
    render_analysis_fairness_profile_response_report,
    ...
    render_bounded_fairness_profile_response_report,
    ...
    render_fairness_profile_multi_response_report, render_fairness_profile_response_report,
...
use formal_verification_lab::recurrence::{analyze_recurrence, analyze_recurrence_with_limits};
use formal_verification_lab::reduction::{audit_sleep_set_reduction, IndependenceRelation};
use formal_verification_lab::report::{
    render_bounded_recurrence_report, render_deadlock_report, render_reachability_report,
    render_recurrence_report, render_report,
};
use formal_verification_lab::response::{
    ...
    check_response_with_product_limits, ResponseProperty, ResponseStatus,
};
use formal_verification_lab::response_examples::{
    finite_pending_request_grant_protocol, request_grant_protocol, unfair_request_grant_protocol,
};
...
fn run(args: Vec<String>) -> Result<ExitCode, String> {
    match args.as_slice() {
        ...
        [command, rest @ ..] if command == "mu" => mu_command(rest),
        _ => Err(usage()),
    }
}
...
fn scc_command(args: &[String]) -> Result<ExitCode, String> {
    match args {
        ...
        [example] => Err(format!(
            "unknown SCC example '{example}'; expected counter, traffic-light, or file <path>"
        )),
        _ => Err(usage()),
    }
}
...
fn run_recurrence_file(path: &str, option_args: &[String]) -> Result<ExitCode, String> {
    ...
    let limits = parse_limits(option_args)?;
    let result =
        analyze_recurrence_with_limits(&model, limits).map_err(|error| error.to_string())?;
    print!(
        "{}",
        render_bounded_recurrence_report(model.name(), &result)
    );
    Ok(match result.outcome {
        BoundedOutcome::Conclusive(_) => ExitCode::SUCCESS,
...
fn response_command(args: &[String]) -> Result<ExitCode, String> {
    ...
    match query.as_str() {
        ...
        "request-grant-terminal" => run_response(
            finite_pending_request_grant_protocol().map_err(|error| error.to_string())?,
            single_response_property()?,
            option_args,
        ),
        ...
        _ => Err(format!(
            "unknown response query '{query}'; expected request-grant, request-grant-unfair, request-grant-terminal, dual-grant, dual-grant-unfair-b, or dual-grant-terminal-b"
        )),
    }
}
...
where
    S: Clone + Eq + std::hash::Hash + std::fmt::Debug,
{
    let has_fairness = option_args
        .iter()
        .any(|arg| arg == "--weak-fair-action" || arg == "--strong-fair-action");
    if has_fairness {
        ...
        if options.has_model_limits {
            ...
            let result = check_response_with_fairness_profile_and_limits(
                &model,
                &property,
        ...
        if options.has_product_limits {
            let result = check_response_with_fairness_profile_and_product_limits(
                &model,
                &property,
                &options.fairness_profile,
        ...
        let result =
            check_response_with_fairness_profile(&model, &property, &options.fairness_profile)
                .map_err(|error| error.to_string())?;
        print!(
            "{}",
            render_fairness_profile_response_report(
                model.name(),
                &result,
                &options.fairness_profile,
            )
...
fn ctl_command(args: &[String]) -> Result<ExitCode, String> {
    match args {
        [command, path, expression, option_args @ ..] if command == "file" => {
            run_ctl_file(path, expression, option_args)
        }
        [query, ..] => Err(format!(
            "unknown CTL query '{query}'; expected 'file <path> <expression> [limits]'"
        )),
        _ => Err(usage()),
    }
}
...
fn run_ctl_file(path: &str, expression: &str, option_args: &[String]) -> Result<ExitCode, String> {
    ...
    let limits = parse_limits(option_args)?;
    let result = check_declarative_ctl_text_with_limits(&document, expression, limits)
        .map_err(|error| error.to_string())?;
    print!(
        "{}",
        render_bounded_declarative_ctl_report(document.model().name(), &result)
    );

    Ok(match result.evaluation.outcome {
        BoundedOutcome::Conclusive(BoundedCtlStatus::Satisfied) => ExitCode::SUCCESS,
        BoundedOutcome::Conclusive(BoundedCtlStatus::Violated) => ExitCode::from(14),
        BoundedOutcome::Inconclusive(_) => ExitCode::from(3),
    })
}
...
fn mu_command(args: &[String]) -> Result<ExitCode, String> {
    match args {
        ...
        [command, action, model_path, expression, certificate_path]
            if command == "certificate" && action == "create" =>
        {
            run_mu_certificate_create(model_path, expression, certificate_path)
        }
        [command, action, model_path, expression, certificate_path]
            if command == "certificate" && action == "verify" =>
        {
...
fn run_mu_certificate_verify(
    ...
    println!("MU PARITY CERTIFICATE: VERIFIED");
    println!("model: {}", document.model().name());
    println!("formula: {}", certificate.formula);
    println!("certificate: {certificate_path}");
    Ok(ExitCode::SUCCESS)
