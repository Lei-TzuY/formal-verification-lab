...
use crate::exact_state::{
    BoundedExactStateResult, ExactStateBackend, ExactStateEvidence, ExactStateResult,
    ExactStateStatus,
};
use std::fmt::Write;

/// Render a stable line-oriented report for the exact-state frontend.
pub fn render_exact_state_report(model_name: &str, result: &ExactStateResult) -> String {
...
/// Render a bounded exact-state result while keeping resource-cutoff
/// incompleteness distinct from a conclusive property result.
pub fn render_bounded_exact_state_report(
    model_name: &str,
